...
def get_modul_dirs(require_modul_network=False):
    ...
    for d in os.listdir(MODULS_PATH):
        ...
        if os.path.isdir(full_dir):
            logic = os.path.exists(os.path.join(full_dir, 'game_logic.py'))
            netmod = os.path.exists(os.path.join(full_dir, 'modul_network.py'))
            if logic and (not require_modul_network or netmod):
                mods.append(d)
    return mods
...
class Game:
    def __init__(self, screen = None, screen_width = 1200, screen_height = 800):
        ...
        pygame.display.set_caption("Unknown World: Infinite")
        ...
        self.create_slots = [
            {'type': 'player', 'id': 1, 'name': 'Host (P1)', 'pos_x': 0, 'pos_y': 0},
            None, None, None
        ]
        self.join_input_error_text = ""
        self.join_ip_input = ""
        ...
        self.modul_select_active = False
        self.create_modul_select_active = False
    ...
    def handle_events(self):
        ...
        if self.state == GameState.JOIN_INPUT_MENU:
            for e in events:
    ...
    def handle_mouse_click(self, pos):
        print(f"[DEBUG] Mouse clicked at: {pos}, state: {self.state}")
        if self.state == GameState.JOIN_INPUT_MENU:
            self.handle_join_input_mouse_click(pos)
        elif self.state == GameState.CLIENT_MENU:
            self.handle_client_menu_mouse_click(pos)
        elif self.state == GameState.MULTIPLAYER_MENU:
            self.handle_multiplayer_menu_click(pos)
        elif self.state == GameState.CREATE_MULTIPLAYER_MENU:
            if self.is_client and hasattr(self, "join_leave_btn") and self.join_leave_btn.collidepoint(pos):
        ...
        if self.state == GameState.MAIN_MENU:
            ...
            self.handle_create_multiplayer_menu_click(pos)
        elif self.state == GameState.JOIN_INPUT_MENU:
            self.handle_join_input_mouse_click(pos)
        elif self.state == GameState.CLIENT_MENU:
            self.handle_client_menu_mouse_click(pos)
    ...
    def handle_join_input_mouse_click(self, pos):
        ...
        if connect_btn_rect.collidepoint(mx, my):
            ip_raw = self.textinput_ip.value.strip()
            if not ip_raw:
                self.join_input_error_text = "IP kiritilmadi!"
            ...
            if ":" in ip_raw:
                ...
                try:
                    ...
                    self.join_input_error_text = "Port noto'g'ri!"
                    return
                ip = ip_part.strip()
            else:
                ip = ip_raw
            ...
            if not re.match(ip_pattern, ip):
                self.join_input_error_text = "IP format noto'g'ri!"
            ...
            if not success:
                ...
                self.join_input_error_text = "Host topilmadi yoki ulanib bo'lmadi!"
                self.is_client = False
                return
            self.session.add_player({"name": client_name, "ip": ip, "role": "client"})
            print(f"[JOIN] ✅ Ulandi. Slots: {self.session.get_state()}")
            self.state = GameState.CLIENT_MENU
            return
    ...
    def handle_client_menu_mouse_click(self, pos):
        if hasattr(self, "join_leave_btn") and self.join_leave_btn.collidepoint(pos):
            if self.client:
                self.client.disconnect()
                self.client = None
            self.is_client = False
    ...
    def handle_play_menu_click(self, pos):
        ...
        if self.modul_select_active:
            moduls_list = get_modul_dirs(require_modul_network=False)
            center_x = self.screen_width // 2
            start_y = 70
            btn_height = 30
            for i, modul_name in enumerate(moduls_list):
                btn_rect = pygame.Rect(center_x - 120, start_y + i * (btn_height + 6), 240, btn_height)
        ...
        for modul_name, modul_rect in self.modul_btn_rects:
            if modul_rect.collidepoint(pos):
                self.modul_select_active = not self.modul_select_active
                return
    ...
    def handle_multiplayer_menu_click(self, pos):
        ...
        if create_rect.collidepoint(mx, my):
            print("[MULTIPLAYER] Create bosildi (keyingi bosqichda server ochiladi)")
            self.state = GameState.CREATE_MULTIPLAYER_MENU

        elif join_rect.collidepoint(mx, my):
            print("[MULTIPLAYER] Join bosildi (keyingi bosqichda serverga ulaniladi)")
            self.state = GameState.JOIN_INPUT_MENU
    ...
    def handle_create_multiplayer_menu_click(self, pos):
        ...
        if create_btn_rect.collidepoint(mx, my):
            ...
            try:
                if not hasattr(self, "host_server") or not self.host_server:
                    ...
                    print(f"[GAME] self.host_server={self.host_server}")
                    self.host_server.start()
                    print(f"[GAME] HostServer start() chaqirildi, is_running={self.host_server.is_running}")
                    self.host_port = self.host_server.port
                    print(f"[CREATE] ✅ HostServer listening on {self.host_ip}:{self.host_port}")
                    print(
                        f"[CREATE] HostServer is_running={self.host_server.is_running}, socket={self.host_server.sock}")
                ...
                print(f"[CREATE] IP:Port = {self.host_ip}:{self.host_port}")
                print(f"[CREATE] Listening holati: {self.host_server.is_running}")

                if not self.host_server.is_running:
                    print("[CREATE] ❌ Server listening emas! Sabab yuqoridagi [HOST] Failed to start logda.")
                    self.host_server = None
                    self.is_host = False
                    self.create_multiplayer_error = f"Server ochilmadi! Port band yoki xato: {self.host_port}. Qayta urining yoki boshqa port tanlang."
                else:
                    self.create_multiplayer_error = ""
            except Exception as e:
                print(f"[CREATE] ❌ HostServer xatolik: {e}")
                self.is_host = False
                self.host_server = None
                self.host_port = None
                self.host_ip = None
        ...
        if self.create_modul_select_active:
            moduls_list = get_modul_dirs(require_modul_network=True)
            center_x = self.screen_width // 2
            start_y = 70
            btn_height = 30
            for i, modul_name in enumerate(moduls_list):
                btn_rect = pygame.Rect(center_x - 120, start_y + i * (btn_height + 6), 240, btn_height)
                if btn_rect.collidepoint(pos):
                    self.selected_create_modul = modul_name
                    self.create_modul_select_active = False
                    return
            self.create_modul_select_active = False
            return

        if hasattr(self, 'create_modul_select_btn') and self.create_modul_select_btn.collidepoint(pos):
            self.create_modul_select_active = not self.create_modul_select_active
            return
        ...
        if self.is_host and hasattr(self, "delete_slot_btns"):
            for idx, del_btn_rect in self.delete_slot_btns:
                if del_btn_rect.collidepoint(pos):
                    self.session.remove_client(idx)
                    print(f"[HOST] Client {idx} removed (slot kicked)")
                    return
    ...
    def _do_loading_from_slots(self):
        try:
            ...
            sel_mod = getattr(self, "loading_selected_modul", "default")
            LoadingScreen.start_game_from_loading(self, self.loading_selected_slots, sel_mod)
    ...
    def render_create_multiplayer_menu(self):
        ...
        host_port = getattr(self, "host_port", 7777) if getattr(self, "host_server", None) else 7777
        ip_text = self.small_font.render(f"Your IP: {host_ip}:{host_port}", True, (255, 255, 0))
        self.screen.blit(ip_text, (center_x - ip_text.get_width() // 2, 30))

        if hasattr(self, "create_multiplayer_error") and self.create_multiplayer_error:
            error_text = self.small_font.render(self.create_multiplayer_error, True, (255, 80, 80))
            self.screen.blit(error_text, (center_x - 130, 65))

        status_str = "🟢 Listening for players..." if getattr(self, "host_server", None) and getattr(self.host_server,
                                                                                                    "is_running",
                                                                                                    False) else "🔴 Not Listening"
        status_color = (255, 255, 0) if "🟢" in status_str else (255, 60, 60)
        print(f"[CREATE MENU] Status: {status_str}, IP:Port={host_ip}:{host_port}")

        status_text = self.small_font.render(status_str, True, status_color)
        self.screen.blit(status_text, (center_x - status_text.get_width() // 2, 30 + ip_text.get_height() + 8))

        modul_btn_width, modul_btn_height = 140, 32
        modul_btn_y = 30 + ip_text.get_height() + status_text.get_height() + 18
        self.create_modul_select_btn = pygame.Rect(center_x - modul_btn_width // 2, modul_btn_y, modul_btn_width,
                                                   modul_btn_height)
        pygame.draw.rect(self.screen, (120, 75, 195), self.create_modul_select_btn, border_radius=8)
        btn_text = f"Modul: {self.selected_create_modul.title() if self.selected_create_modul else 'default'}"
        self.screen.blit(self.small_font.render(btn_text, True, WHITE), self.create_modul_select_btn.move(10, 3))

        slot_width, slot_height, slot_gap = 180, 60, 18
        slot_start_y = modul_btn_y + modul_btn_height + 18
        session_slots = self.session.get_state() if hasattr(self, "session") else []
        max_slots = 4
        alive_slots = [s for s in session_slots if s]

        self.delete_slot_btns = []
        for i in range(max_slots):
            slot_rect = pygame.Rect(center_x - slot_width // 2, slot_start_y + i * (slot_height + slot_gap), slot_width,
                                    slot_height)
            if i == 0:
                name = alive_slots[0]["name"] if alive_slots else "Host (P1)"
                color = (24, 134, 42)
                pygame.draw.rect(self.screen, color, slot_rect, border_radius=12)
                slot_text = self.small_font.render(name, True, WHITE)
                self.screen.blit(slot_text, slot_text.get_rect(center=slot_rect.center))
            else:
                real_slot = alive_slots[i] if i < len(alive_slots) else None
                name = real_slot["name"] if real_slot else "Empty"
                color = (60, 150, 230) if real_slot else (180, 180, 180)
                pygame.draw.rect(self.screen, color, slot_rect, border_radius=12)
                slot_text = self.small_font.render(name, True, WHITE if real_slot else (60, 60, 60))
                self.screen.blit(slot_text, slot_text.get_rect(center=slot_rect.center))
                if real_slot and self.is_host:
                    del_btn_rect = pygame.Rect(slot_rect.right - 30, slot_rect.top + 10, 22, 22)
                    pygame.draw.rect(self.screen, (220, 50, 50), del_btn_rect, border_radius=6)
                    x_text = self.small_font.render("X", True, WHITE)
                    self.screen.blit(x_text, x_text.get_rect(center=del_btn_rect.center))
                    self.delete_slot_btns.append((i, del_btn_rect))

        slots_area_bottom = slot_start_y + max_slots * (slot_height + slot_gap)
        create_btn_width, create_btn_height = 180, 50
        create_btn_y = slots_area_bottom + 30
        ...
        create_text = self.small_font.render("Start Game", True, WHITE)
        ...
        if self.create_modul_select_active:
            moduls_list = get_modul_dirs(require_modul_network=True)
            dropdown_rect = pygame.Rect(center_x - 120, modul_btn_y + modul_btn_height + 6, 240, len(moduls_list) * 36)
            pygame.draw.rect(self.screen, (90, 90, 90), dropdown_rect, border_radius=12)
            for i, modul_name in enumerate(moduls_list):
                btn_rect = pygame.Rect(center_x - 120, modul_btn_y + modul_btn_height + 6 + i * 36, 240, 30)
                color = (60, 180, 235) if self.selected_create_modul == modul_name else (120, 120, 120)
                pygame.draw.rect(self.screen, color, btn_rect, border_radius=6)
                self.screen.blit(self.small_font.render(modul_name, True, WHITE), btn_rect.move(10, 4))
    ...
    def render_join_multiplayer_menu(self):
        ...
        if self.join_ip_active:
            ...
            self.screen.blit(hint, (center_x + 70, 255))

        if self.join_input_error_text:
            error_text = self.small_font.render(self.join_input_error_text, True, (255, 80, 80))
            self.screen.blit(error_text, (center_x - 80, 320))
    ...
    def render_client_menu(self):
        self.screen.fill((50, 120, 190))
        center_x = self.screen_width // 2
        host_ip, host_port = (self.client.host_ip, self.client.port) if getattr(self, "client", None) else ("", "")
        ip_text = self.small_font.render(f"Host IP: {host_ip}:{host_port}", True, (255, 255, 0))
        self.screen.blit(ip_text, (center_x - ip_text.get_width() // 2, 30))
        status_text = self.small_font.render("🟢 Connected", True, (60, 255, 60))
        self.screen.blit(status_text, (center_x - status_text.get_width() // 2, 30 + ip_text.get_height() + 8))
        slot_y = 30 + ip_text.get_height() + status_text.get_height() + 24
        modul_text = self.small_font.render(f"Modul: {self.selected_create_modul.title()}", True, WHITE)
        self.screen.blit(modul_text, (center_x - modul_text.get_width() // 2, slot_y))
        slot_width, slot_height, slot_gap = 180, 60, 18
        slot_start_y = slot_y + modul_text.get_height() + 18
        session_slots = self.session.get_state() if hasattr(self, "session") else []
        max_slots = 4
        alive_slots = [s for s in session_slots if s]
        for i in range(max_slots):
            ...
            slot_rect = pygame.Rect(center_x - slot_width // 2, slot_start_y + i * (slot_height + slot_gap), slot_width,
                                    slot_height)
            name = real_slot["name"] if real_slot else ("Host (P1)" if i == 0 else "Empty")
            color = (24, 134, 42) if i == 0 else (60, 150, 230) if real_slot else (180, 180, 180)
            pygame.draw.rect(self.screen, color, slot_rect, border_radius=12)
            slot_text = self.small_font.render(name, True, WHITE if real_slot else (60, 60, 60))
            self.screen.blit(slot_text, slot_text.get_rect(center=slot_rect.center))
        slots_area_bottom = slot_start_y + max_slots * (slot_height + slot_gap)
        leave_btn_width, leave_btn_height = 180, 50
        leave_btn_y = slots_area_bottom + 36
        leave_btn_rect = pygame.Rect(center_x - leave_btn_width // 2, leave_btn_y, leave_btn_width, leave_btn_height)
        pygame.draw.rect(self.screen, (200, 80, 80), leave_btn_rect, border_radius=12)
        leave_text = self.small_font.render("Leave", True, WHITE)
        self.screen.blit(leave_text, leave_text.get_rect(center=leave_btn_rect.center))
        self.join_leave_btn = leave_btn_rect
    ...
    def render_play_menu(self):
        ...
        modul_btn_width, modul_btn_height = 140, 32
        self.modul_btn_rects = []
        modul_btn_rect = pygame.Rect(center_x - modul_btn_width // 2, modul_btn_y, modul_btn_width, modul_btn_height)
        pygame.draw.rect(self.screen, (120, 190, 50), modul_btn_rect, border_radius=8)
        btn_text = f"Modul: {self.selected_modul.title() if self.selected_modul else 'default'}"
        self.screen.blit(self.small_font.render(btn_text, True, WHITE), modul_btn_rect.move(10, 3))
        self.modul_btn_rects.append((self.selected_modul, modul_btn_rect))
        ...
        if self.modul_select_active:
            ...
            for i, modul_name in enumerate(moduls_list):
                btn_rect = pygame.Rect(center_x - 120, modul_btn_y + modul_btn_height + 6 + i * 36, 240, 30)
                color = (60, 180, 70) if self.selected_modul == modul_name else (120, 120, 120)
                pygame.draw.rect(self.screen, color, btn_rect, border_radius=6)
                self.screen.blit(self.small_font.render(modul_name, True, WHITE), btn_rect.move(10, 4))
