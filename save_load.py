...
def load_game_data(game_mode, save_name, progress_callback=None):
    db_path = get_save_path(game_mode, save_name)
    ...
    if game_mode.name != mode:
        print(
            f"[ERROR] Fayl '{save_name}' {mode} rejimida saqlangan, lekin siz {game_mode.name} rejimida ochmoqchisiz!")
        conn.close()
        return None
    data["meta"] = {
        "mode": mode,
