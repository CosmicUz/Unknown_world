...
def get_save_path(save_name, modul_name="ProtectBase"):
...
def get_last_session_path(game_mode, modul_name="ProtectBase"):
    return get_save_path("autosave", modul_name)


def save_last_session(game, modul_name="ProtectBase"):
...
def create_tables(conn):
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS base (
            id INTEGER PRIMARY KEY,
            position_x REAL, position_y REAL,
            health INTEGER, max_health INTEGER, size INTEGER,
            level INTEGER, zombie_kills INTEGER, weapon_type TEXT,
            last_fire_time INTEGER, fire_rate INTEGER, damage INTEGER,
            chain_radius INTEGER, active INTEGER
        )
    ''')
    c.execute('''
...
def save_game(game, save_name, game_mode, modul_name="ProtectBase"):
    print(f"[save_game][ProtectBase] Saving '{save_name}' for module '{modul_name}' (players: {len(getattr(game, 'players', []))}, base_present={hasattr(game, 'base') and game.base is not None})")
    ...
    # Base
    if hasattr(game, 'base') and game.base:
        c.execute('''
            INSERT INTO base VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            game.base.position.x, game.base.position.y,
            game.base.health, game.base.max_health, game.base.size,
            game.base.level, game.base.zombie_kills, game.base.weapon_type.name,
            game.base.last_fire_time, game.base.fire_rate, game.base.damage,
            game.base.chain_radius, int(game.base.active)
        ))
...
def try_load_game_any_mode(save_name, modul_name="ProtectBase"):
...
def load_game_data(save_name, progress_callback=None, modul_name="ProtectBase"):
    db_path = get_save_path(save_name, modul_name)
    ...
    # Base
    base_row = c.execute('SELECT * FROM base WHERE id=1').fetchone()
    if base_row:
        (
            _, bx, by, bhealth, bmax_health, bsize, blevel, bzombie_kills,
            bweapon_type, blast_fire_time, bfire_rate, bdamage, bchain_radius, bactive
        ) = base_row
        data["base"] = {
            "position": [bx, by],
            "health": bhealth,
            "max_health": bmax_health,
            "size": bsize,
            "level": blevel,
            "zombie_kills": bzombie_kills,
            "weapon_type": bweapon_type,
            "last_fire_time": blast_fire_time,
            "fire_rate": bfire_rate,
            "damage": bdamage,
            "chain_radius": bchain_radius,
            "active": bool(bactive)
        }
    else:
        data["base"] = None
...
def load_from_data(game, data):
    ...
    # --- Base ---
    base_data = data.get("base", None)
    if base_data:
        game.base = Base(Vector2(*base_data.get("position", [0, 0])))
        game.base.health = safe_int(base_data.get("health", 1000))
        game.base.max_health = safe_int(base_data.get("max_health", 1000))
        game.base.size = safe_int(base_data.get("size", 80))
        game.base.level = safe_int(base_data.get("level", 1))
        game.base.zombie_kills = safe_int(base_data.get("zombie_kills", 0))
        game.base.weapon_type = safe_enum(WeaponType, base_data.get("weapon_type", "PISTOL"), WeaponType.PISTOL)
        game.base.last_fire_time = safe_int(base_data.get("last_fire_time", 0))
        game.base.fire_rate = safe_int(base_data.get("fire_rate", 1000))
        game.base.damage = safe_int(base_data.get("damage", 100))
        game.base.chain_radius = safe_int(base_data.get("chain_radius", 150))
        game.base.active = safe_bool(base_data.get("active", True))
    else:
        # Create default base if not loaded
        game.base = Base(Vector2(0, 0))
    
    # Connect players to base
    for player in game.players:
        if hasattr(player, 'connect_to_base'):
            player.connect_to_base(game.base, player.id - 1)

    # Assign controls and multiplayer flags so loaded players receive input
    try:
        PLAYER_CONTROLS = {
            1: {
                'up': pygame.K_w,
        ...
        print(f"[load_from_data][ProtectBase] players loaded: {len(game.players)}, multiplayer={is_multiplayer}, base_loaded={bool(base_data)}")
        idx = 1
        for p in game.players:
            if isinstance(p, HelperBot):
                ...
                p.multi_player_mode = is_multiplayer
                p.can_go_down = is_multiplayer
                print(f"[load_from_data][ProtectBase] Assigned controls to player id={p.id}, controls={p.controls}")
                idx += 1
    except Exception as e:
        print(f"[load_from_data][ProtectBase] Failed to assign controls: {e}")
    ...
    # Timers
    current_ticks = pygame.time.get_ticks() if pygame else 0
    game.last_zombie_spawn = current_ticks
    game.next_power_up_time = current_ticks + 5000
...
def list_saved_games(modul_name="ProtectBase"):
...
def delete_save(save_name, modul_name="ProtectBase"):
    db_path = get_save_path(save_name, modul_name)
    if save_name == "autosave":
